...
class BaseSetNew:
    ...
    def __init__(self, playwright_page, sid, websocket_client, session_id, RealMetrics_Width, RealMetrics_Height,
                 Metrics_Width, Metrics_Height, screen_width, screen_height, visualViewport_width, visualViewport_height):
    ...
    async def page_click(self, x, y):
        ...
        x = int(x)
        y = int(y)
        screenOtherHeight = self.screen_height - self.visualViewport_height
        screenButtonHeight = self.RealMetrics_Height - self.Metrics_Height
        await self.my_print("screen_height:", self.screen_width, "screen_height:", self.screen_height, " -visualViewport_height:",
                            self.visualViewport_height, " -screenOtherHeight:", screenOtherHeight," - screenButtonHeight:",screenButtonHeight)
        y = y + screenOtherHeight
        clickX = self.RealMetrics_Width / self.screen_width * x
        clickY = self.RealMetrics_Height / self.screen_height * y
    ...
    async def adImage_click(self, x, y):
        await self.my_print("adImage_click -x:", x, " -y:", y)
        x = int(x)
        y = int(y)
        # 点击的时候，注意区别手机真实的分辨率
        screenOtherHeight = self.screen_height - self.visualViewport_height
        ...
        await self.my_print("screen_height:", self.screen_width, "screen_height:", self.screen_height, " -visualViewport_height:",
                            self.visualViewport_height, " -screenOtherHeight:", screenOtherHeight," - screenButtonHeight:",screenButtonHeight)
        clickY = int(y + int(self.RealMetrics_Height / self.screen_height * screenOtherHeight))
        clickY = clickY - screenButtonHeight
        clickX = int(x)
        clickY = int(clickY)
        clickObject = {"type": "click", "sessionId": self.session_id, "startX": clickX, "startY": clickY}
    ...
    async def page_randomSwipe(self):
        ...
        await self.my_print("判断是否滑动到最底部 - scrollTop:", scrollTop, " - visualViewport_height:", self.visualViewport_height,
                            " - scrollHeight:", scrollHeight)
        if scrollTop <= (self.visualViewport_height * 0.33):
            await self.my_print("接近顶部，向上滑动")
            await self.page_swipeUp()
        else:
            if scrollTop >= (scrollHeight - self.visualViewport_height * 1.33):
